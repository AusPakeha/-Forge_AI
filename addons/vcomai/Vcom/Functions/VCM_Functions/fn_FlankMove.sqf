/*
    Author: Genesis + patch
    Description:
        Generates flanking waypoints.
        Patched to avoid placing waypoints on water.
    Parameter(s):
...
// wrapper to skip waypoints on water
private _safeAddWP = {
    params ["_grp","_pos"];
    if (surfaceIsWater _pos) exitWith {
        [objNull, -1] // Return an array, not just objNull
    };
    private _wp = _grp addWaypoint [_pos,0];
    _wp setWaypointType "MOVE";
    [_grp, (_wp select 1)] setWaypointCompletionRadius 50;
    _wp
};
...
// record assigned WP types per enemy group
...
switch (_WayPointType) do {
    case "Assault": {
        private _finalP = getpos _nearestEnemy;
        private _approachP = _finalP getPos [50 + random 50, random 360];
        _approachP set [2, 0];
        private _wp0 = [_grp,_approachP] call _safeAddWP;
        if (!isNull (_wp0 select 0)) then { _grp setCurrentWaypoint [_grp,(_wp0 select 1)]; };
        [_grp,_finalP] call _safeAddWP;
    };
    case "High": {
        ...
        _finalP set [2,0];
        private _wp0 = [_grp,_highP] call _safeAddWP;
        if (!isNull (_wp0 select 0)) then { _grp setCurrentWaypoint [_grp,(_wp0 select 1)]; };
        [_grp,_finalP] call _safeAddWP;
    };
    case "Low": {
    ...
    case "Retreat": {
        ...
        _MovePosition set [2,0];
        private _finalP = _MovePosition getpos [(random 50),(random 360)];
        _finalP set [2,0];
        private _wp0 = [_grp,_MovePosition] call _safeAddWP;
        if (!isNull (_wp0 select 0)) then { _grp setCurrentWaypoint [_grp,(_wp0 select 1)]; };
        [_grp,_finalP] call _safeAddWP;
    };
    case "Flank": {
        ...
        _finalP set [2,0];
        if (_finalP isEqualTo [0,0,0]) then {_finalP = _RandomLocation;};
        private _wp0 = [_grp,_finalP] call _safeAddWP;
        if (!isNull (_wp0 select 0)) then { _grp setCurrentWaypoint [_grp,(_wp0 select 1)]; };
        [_grp,_myEnemyPos] call _safeAddWP;
    };
    case "FlankL": {
        private _myEnemyPos = getpos _nearestEnemy;
        private _rnd = random 100;
        ...
        if (_myPlaces isEqualTo []) then {_myPlaces = [_positions];};
        if (_myPlacesL isEqualTo []) then {_myPlacesL = [_positionsL];};
        private _RandomArray = _myPlaces call BIS_fnc_selectrandom;
        ...
        _finalP2 set [2,0];
        [_grp,_finalP] call _safeAddWP;
        private _wp0 = [_grp,_finalP2] call _safeAddWP;
        if (!isNull (_wp0 select 0)) then { _grp setCurrentWaypoint [_grp,(_wp0 select 1)]; };
        [_grp,_myEnemyPos] call _safeAddWP;
    };
};
